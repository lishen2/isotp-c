 ...
 * by software programs using this library.
 */
typedef struct IsoTpLink {
  ...
  uint8_t
      send_st_min; /* Separation Time between consecutive frames, unit millis */
  ...
  uint32_t receive_timer_cr; /* Time until transmission of the next
                                ConsecutiveFrame N_PDU start at sending FC,
                                receive CF end at receive FC */
 ...
 * @param sendbuf A pointer to an area in memory which can be used as a buffer
 * for data to be sent.
 * @param sendbufsize The size of the buffer area.
 * @param recvbuf A pointer to an area in memory which can be used as a buffer
 * for data to be received.
 * @param recvbufsize The size of the buffer area.
 * @param isotp_user_get_ms A pointer to a function which returns the current
 * time as milliseconds.
 * @param isotp_user_send_can A pointer to a function which sends a can message.
 * should return ISOTP_RET_OK when success.
 ...
 * Determines whether an incoming message is a valid ISO-TP frame or not and
...
/**
 * @brief Sends ISO-TP frames via CAN, using the ID set in the initialising
 * function.
 *
...
/**
 * @brief See @link isotp_send @endlink, with the exception that this function
 ...
 * @param payload A pointer to an area in memory where the raw data is copied
 * from.
 * @param payload_size The size of the received (raw) CAN data.
 * @param out_size A reference to a variable which will contain the size of the
 * actual (parsed) data.
 *
 * @return Possible return values:
